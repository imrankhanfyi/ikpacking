    ...
    <Modal title={kit ? 'Edit kit' : 'New kit'} onClose={onClose} wide>
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="max-h-[60vh] overflow-y-auto border-[1.5px] border-[#ddd] rounded">
            {categories.map(cat => (
              <div key={cat}>
                <div className="px-3 py-1.5 bg-[#f9f8f6] font-mono text-[10px] uppercase tracking-[2px] text-[#bbb] sticky top-0 col-span-2 border-b border-[#eee]">
                  {cat}
                </div>
                  ...
                  {grouped[cat].map(m => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-2 px-3 py-2 cursor-pointer select-none border-b border-[#eee] ${checked ? 'bg-[#f5f3ef]' : 'hover:bg-[#fafafa]'}`}
                      >
                        <span className={`w-4 shrink-0 text-center text-xs font-bold ${checked ? 'text-[#2d2d2d]' : 'text-[#ddd]'}`}>
                          {checked ? '✓' : '○'}
                        </span>
                        <span className="flex-1 text-sm text-[#2d2d2d] truncate">{m.name}</span>
                        {checked && ki && (
                          <input
                            type="number"
                            min={1}
                            ...
                            className="w-12 shrink-0 bg-white border-[1.5px] border-[#ddd] rounded px-1 py-1 text-xs text-[#2d2d2d] text-center focus:outline-none focus:border-[#2d2d2d]"
                          />
                        )}
                      </div>
                    )
